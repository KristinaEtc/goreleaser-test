...
type Client interface {
	CreateRelease(ctx *Context, body string) (releaseID int, err error)
	CreateFile(ctx *Context, content bytes.Buffer, path string) (err error)
	Upload(ctx *Context, releaseID int, name string, file *os.File) (err error)
}
...
func NewGitHub(ctx *Context) (Client, error) {
	...
	if ctx.Config.GitHubURLs.API != "" {
		log.Println("\n\n============github.RepositoryRelease=", ctx.Config.GitHubURLs.API)
...
func (c *githubClient) CreateRelease(ctx *Context, body string) (releaseID int, err error) {
	...
	log.Println("--------------------releaseTitle=", releaseTitle)
